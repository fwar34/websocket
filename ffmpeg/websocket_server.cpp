...
struct WebSocketClient {
    ...
    bool init_sent;            ///< 是否已发送视频分辨率 init 消息
    std::string receive_buffer; ///< 接收缓冲区（累积分片数据，直到完整帧）
    
    WebSocketClient(SOCKET s) : sock(s), handshake_done(false), connected(true), init_sent(false) {}
};
 ...
 * @brief 设置视频分辨率信息
 * 
 * 主线程调用此方法设置视频宽高。worker 线程在检测到 video_info_changed_ 标志后，
 * 会向每个已握手但尚未收到 init 消息的客户端发送一条文本帧：
 *   {"type":"init","width":W,"height":H}
 * 浏览器收到后据此设置 Canvas 尺寸。
 */
void WebSocketServer::set_video_info(int width, int height) {
    video_info_width_ = width;
    video_info_height_ = height;
}
...
void WebSocketServer::worker_thread(int worker_id) {
    ...
    while (running_) {
        // 0. 发送 init 消息给新连接的客户端（每次循环都检查）
        if (video_info_width_ > 0) {
            ...
            for (auto& c : clients_copy) {
                if (c->connected && c->handshake_done && !c->init_sent) {
                    std::string init_msg = "{\"type\":\"init\",\"width\":" +
                        std::to_string(video_info_width_) + ",\"height\":" +
                        std::to_string(video_info_height_) + "}";
                    send_websocket_frame(c->sock,
